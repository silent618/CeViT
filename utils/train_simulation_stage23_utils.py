...
def get_test_loss(net, criterion_copula, criterion_regression, criterion_classification, data_loader, gamma12, gamma34,
    ...
    with torch.no_grad():
        for data in data_loader:
            ...
            # Separate labels
            labels_left_AL = data['Y_left_al'].float().unsqueeze(0).to(device)
            labels_right_AL = data['Y_right_al'].float().unsqueeze(0).to(device)
            labels_left_high = data['Y_left_myopia'].float().unsqueeze(0).to(device)
            labels_right_high = data['Y_right_myopia'].float().unsqueeze(0).to(device)
...
# test_stage
def test_stage(net, criterion_copula, criterion_regression, criterion_classification, test_loader, gamma12, gamma34,
...
# Train model
def train_model(model, criterion_copula, criterion_regression, criterion_classification, optimizer, scheduler, dl_train,
                dl_val, dl_test,
    ...
    for epoch in range(num_epochs):  # [0, num_epochs-1]
        ...
        for data in tqdm(dl_train):  # i 从0开始
            ...
            if i % 100 == 99:
                ...
                val_loss_summary = get_test_loss(model, criterion_copula, criterion_regression,
        ...
        # 下方使用item，把cuda里的tensor的数值部分提取到cpu里，这种方法比什么tensor.to("cpu")来得直接
        train_loss_summary = get_test_loss(model, criterion_copula, criterion_regression, criterion_classification,
                                           dl_train, gamma12, gamma34, gamma3412, sigma1, sigma2, gamma, device)
        val_loss_summary = get_test_loss(model, criterion_copula, criterion_regression, criterion_classification,
                                         dl_val, gamma12, gamma34, gamma3412, sigma1, sigma2, gamma, device)
        test_loss_summary = get_test_loss(model, criterion_copula, criterion_regression, criterion_classification,
        ...
        get_test_loss(model, criterion_copula, criterion_regression, criterion_classification, dl_val, gamma12, gamma34,
    ...
    get_test_loss(model, criterion_copula, criterion_regression, criterion_classification, dl_train, gamma12, gamma34,
                  gamma3412, sigma1, sigma2, gamma, device)["avg_loss"]
    avg_val_loss_best_model = \
    get_test_loss(model, criterion_copula, criterion_regression, criterion_classification, dl_val, gamma12, gamma34,
                  gamma3412, sigma1, sigma2, gamma, device)["avg_loss"]
    avg_test_loss_best_model = \
    get_test_loss(model, criterion_copula, criterion_regression, criterion_classification, dl_test, gamma12, gamma34,
                  gamma3412, sigma1, sigma2, gamma, device)["avg_loss"]
...
def plot_loss(batch_num_per_epoch, train_loss, val_loss, test_loss, work_id, seed, fold, X_length, batch_size, num_epochs):
