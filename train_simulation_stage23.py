...
from utils.train_simulation_stage23_utils import train_model, plot_loss, test_stage
from dataloader.dataloader import get_datasets
from utils.loss import calculate_sigma_hat
from utils.loss import parametricLoss
# from utils.loss_old import parametricLoss
...
class CoViT(nn.Module):
    def __init__(self, device='cuda:6'):
...
if __name__ == "__main__":
    ...
    for fold in range(k):
        ...
        train_path = f'train_data_save/WorkID{work_id}_RC_{model_name}_Train_Data_seed{seed}_fold{fold}_X_length{X_length}_batchsize{batch_size}_num_epoch{num_epochs}.csv'
        train_dataset = get_datasets(train_path, '/home/netzone22/yhliu/CoViT/')
        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=2,
                                  pin_memory=True)

        val_path = f'train_data_save/WorkID{work_id}_RC_{model_name}_Val_Data_seed{seed}_fold{fold}_X_length{X_length}_batchsize{batch_size}_num_epoch{num_epochs}.csv'
        val_dataset = get_datasets(val_path, '/home/netzone22/yhliu/CoViT/')
        val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, drop_last=True)

        test_path = f'train_data_save/WorkID{work_id}_RC_{model_name}_Test_Data_seed{seed}_fold{fold}_X_length{X_length}_batchsize{batch_size}_num_epoch{num_epochs}.csv'
        test_dataset = get_datasets(test_path, '/home/netzone22/yhliu/CoViT/')
        ...
        resnet_adapter, train_loss_batchwise, val_loss_batchwise, test_loss_batchwise = train_model(covit,
                                                                                                    criterion_copula,
                                                                                                    criterion_regression,
                                                                                                    criterion_classification,
                                                                                                    optimizer,
                                                                                                    exp_lr_scheduler, train_loader,
        ...
        torch.save(resnet_adapter.module.state_dict(), copula_PATH)  # 模型提取 state_dict状态字典 并保存
        ...
        loss_summary = test_stage(oucopula_test, criterion_copula, criterion_regression, criterion_classification,
                                  test_loader, gamma12, gamma34, gamma3412, sigma1, sigma2, gamma, device, work_id,
