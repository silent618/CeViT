...
from modeling.module1 import CoViT_Adapter
from utils.train_stage23_utils import train_model_stage23, plot_loss_stage23, test_stage_stage23
...
if __name__ == "__main__":
    for seed in seed_list:
        ...
        for fold, (train_idx, test_idx) in enumerate(kfold.split(np.arange(X_length))):
            ...
            # network参数
            covit = CoViT_Adapter(device=device, rank_LoRa=rank_LoRa).to(device)
            gpu_ids = ['cuda:1', 'cuda:2', 'cuda:3']
            ...
            # ====== 测试模型 ======
            net_test = CoViT_Adapter(device).to(device)  # 载入模型之前先初始化
        ...
        for fold in range(k):
            ...
            # ====== 测试模型 ======
            oucopula_test = CoViT_Adapter(device).to(device)  # 载入模型之前先初始化
